...
public class MasterModuleList {
    ...
    private static void loadModulesFromFile() {
        try (InputStream inputStream = MasterModuleList.class.getClassLoader().getResourceAsStream("moduledata.txt")) {
            if (inputStream == null) {
            ...
            try (BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream, StandardCharsets.UTF_8))) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (parts.length == 4) {
                        modules.add(new Mod(parts[1].trim(), parts[3].trim(), Integer.parseInt(parts[2].trim()), parts[0].trim()));
                    } else {
